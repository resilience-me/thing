...
func SyncTrustlineOut(session main.Session) {
    ...
    if err := trustlines.ValidateCounter(datagram); err != nil {
        log.Printf("Counter validation failed in SyncTrustlineOut for user %s: %v", datagram.Username, err)
    ...
    if err := db_trustlines.SetCounter(datagram, datagram.Counter); err != nil {
        log.Printf("Error updating counter in SyncTrustlineOut for user %s: %v", datagram.Username, err)
