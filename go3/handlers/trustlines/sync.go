...
func PrepareDatagramForSync(datagram *main.Datagram) (*main.Datagram, error) {
    ...
    if err != nil {
        return nil, fmt.Errorf("failed to retrieve sync status for user %s: %v", datagram.Username, err)
    }
    ...
    if isSynced {
        ...
        if err != nil {
            return nil, fmt.Errorf("error getting trustline for user %s: %v", datagram.Username, err)
        }
