...
This implementation will then use no encryption of the messages, besides for exchanging account secret keys. It will use (mostly) no encryption since encryption isn't strictly needed to run Ripple. It is easy to add. Note that assuming account-to-account encryption, the account identifier has to be plaintext anyway.
