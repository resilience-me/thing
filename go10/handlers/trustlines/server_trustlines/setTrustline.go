...
func SetTrustline(session main.Session) {
    ...
    syncIn := types.BytesToUint32(datagram.Arguments[4:8])

    if syncIn > prevSyncIn {
        handleTrustlineUpdate(session, syncIn)
    } else {
        handleTimestampOnly(session)
    }
}

func handleTrustlineUpdate(session main.Session, syncIn uint32) {
    datagram := session.Datagram
    ...
    if err := db_trustlines.SetTrustlineIn(datagram, trustlineAmount); err != nil {
    ...
    log.Printf("Trustline, sync_in, and timestamp updated successfully for user %s.", datagram.Username)

    // Prepare and send the datagram to sync the peer's out counter
    if err := handlers.PrepareAndSendDatagram(commands.ServerTrustlines_SetSyncOut, datagram.PeerUsername, datagram.PeerServerAddress, datagram.Username, types.Uint32ToBytes(syncIn)); err != nil {
